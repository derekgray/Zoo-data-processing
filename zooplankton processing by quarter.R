...
######1. winter, spring, summer, fall: A time series by quarter (winter=DJF, spring=MAM, summer=JJA, fall=SON)
...
#this code shifts all months back 1 so that quarters align DJF, MAM, JJA, SON (easiest way to accomplish this)
months<-as.numeric(substr(allzoocommondepths$Date1,6,7))+1
months[which(months==13)]<-1
years<-as.numeric(substr(allzoocommondepths$Date1,1,4))
...
allzoocommondepths$Date1<-as.Date(paste(years,months,days,sep="-"))

#summarize the data into density for each species by date
library(doBy)
...
#examples for working with the data
#write.csv(fall,"fall.csv",row.names=F) #how to write a csv
#plot(as.vector(fall[,3])~fall$year, main=names(fall)[3]) #how to plot data e.g. 3rd column of fall
